...
namespace TradingBot
{
    public class SpykeStrategy : IStrategy
    {
        public IStrategy.StrategyResultType Process(MarketInstrument instrument, TradeData tradeData, Quotes quotes)
        {
            ...
            {
                // if we got some quotes after spyke but order still not executed - cancel it.
                ...
                if (strategyData.QuotesBuy > 4)
                {
                    ...
                    Logger.Write("{0}: No lack with spyke, cancel order....", instrument.Ticker);
            ...
            else if (tradeData.Status == Status.BuyDone)
            {
                if (candle.Close > tradeData.BuyPrice)
                {
                    ...
                    Logger.Write("{0}: Was bought by spyke. Pending. Close price: {1}. Candle: {2}. Profit: {3}({4}%)", instrument.Ticker, tradeData.SellPrice, JsonConvert.SerializeObject(candle), tradeData.SellPrice - tradeData.BuyPrice, Helpers.GetChangeInPercent(tradeData.BuyPrice, tradeData.SellPrice));
